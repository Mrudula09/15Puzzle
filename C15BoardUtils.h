...
Board * createBoard(int rows, int cols){
	if (rows <= 0 || cols <= 0)
		return NULL;
	Board *board = (struct Board*)malloc(sizeof(Board));
...
int checkBoard(Board * board){
	...
	for (i = 0; i < board->rows; i++)
	{
		for (j = 0; j < board->cols; j++)
		{
			...
			{
				if ((board->rows - 1 != i) && (board->cols != j))
				{
					flag = 0;
					break;
				}
	...
	if (flag == 1 && (board->table)[board->rows - 1][board->cols - 1] == 0)
	{
		flag = 1;
	}
	else
